...
- **Third-party scraping APIs** (Apify, Bright Data, ScrapFly, Decodo, Ayrshare) — resell comment
  access; offload the proxy/maintenance burden but add vendor cost and the **same ToS/legal exposure**
  (you're paying someone to violate TikTok ToS on your behalf).

### Unofficial / GitHub tools — do they exist? (surveyed 2026-07-13)

Yes, but the landscape is **mostly dead and fragile**, and using any of it is a **ToS violation**.

- **`davidteather/TikTok-Api`** (Python) — the one still-maintained credible option (active ~2026-04).
  Has a **`comments` method** (`TikTokApi/api/comment.py`) yielding comment objects (author, nickname,
  likes, replies, text) with amount/offset. Drives a **real browser (Playwright)** and needs an
  `ms_token`; read-only, no authenticated routes.
- Others are stale or signature-service shims: `Evil0ctal/Douyin_TikTok_Download_API` (documents
  msToken/ttwid/X-Bogus/A_Bogus), `EterZ-byte/tiktok-signature-api`, `drawrowfly/tiktok-scraper`
  (old), `networkdynamics/pytok`. **Most GitHub TikTok scrapers are broken in 2026.**
- **Why it's fragile:** TikTok web requires signed params (**msToken, _signature, X-Bogus, A_Bogus,
  ttwid**) generated by a **VM running in the browser** — hence the Playwright dependency, not pure
  HTTP. On top of that: ML-based anti-scraping, encrypted headers, behavioral + real-time fraud
  scoring, and **device trust scores** — fresh/low-trust sessions get rate-limited, empty responses,
  or CAPTCHA even with valid signatures. Standing up a reliable pipeline means Playwright + rotating
  residential proxies + aged/trusted device tokens, and it **breaks whenever TikTok rotates signing**.

**Verdict:** technically possible (davidteather/TikTok-Api reads comments today), but **not for the
productized multi-tenant connector** — it's a TikTok ToS violation, legally/operationally risky, and
too fragile to run as a standing per-merchant pipeline.

**Recommendation:** treat TikTok organic comments as **out of scope for v1** (official path blocked,
unofficial path is ToS + fragile). Track TikTok for *ads/performance* (the existing 1.1.8 connector).
If a specific study genuinely needs TikTok comment text, do a **one-off, research-only** pull behind
the **legal/ToS review gate** (master plan 1.5.2) — never a standing productized scraper.
...
- TikTok unofficial (ToS-violating, research-only) — davidteather/TikTok-Api comment module: https://github.com/davidteather/TikTok-Api/blob/main/TikTokApi/api/comment.py · landscape (most dead in 2026): https://thunderbit.com/blog/best-tiktok-scraper-alternatives
