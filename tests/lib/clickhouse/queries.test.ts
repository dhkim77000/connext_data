...
describe('insertRows', () => {
  it('inserts rows into a ClickHouse table without throwing', async () => {
    ...
    await expect(insertRows('shopify_orders_history', rows)).resolves.not.toThrow()
  })

  it('is a no-op when rows array is empty', async () => {
    await expect(insertRows('shopify_orders_history', [])).resolves.not.toThrow()
  })
})
...
describe('queryRows', () => {
  it('returns typed rows from a query', async () => {
    ...
    const result = await queryRows<Row>(
      'SELECT order_id FROM shopify_orders_history WHERE tenant_id = {tenantId:String}',
      { tenantId: 'abc' }
    )
