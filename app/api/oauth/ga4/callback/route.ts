...
export async function GET(request: Request) {
  ...
  const nonce = cookieStore.get('ga4_oauth_nonce')?.value
  if (!code || !state || state !== nonce) {
    return NextResponse.json({ error: 'Invalid OAuth callback' }, { status: 400 })
  }

  const redirectUri = `${getBaseUrl(request)}/api/oauth/ga4/callback`
  ...
  const propertiesRes = await fetch('https://analyticsadmin.googleapis.com/v1beta/accounts', {
    headers: { Authorization: `Bearer ${access_token}` },
  })
  let displayName = 'Google Analytics 4'
  if (propertiesRes.ok) {
    const { accounts } = (await propertiesRes.json()) as {
      accounts?: Array<{ displayName: string; name: string }>
    }
    const firstAccount = accounts?.[0]
    if (firstAccount?.displayName) displayName = firstAccount.displayName
  }
  ...
  redirect.cookies.delete('ga4_oauth_nonce')
  return redirect
}
